...
//  we are calling intersectObjct on mode, which is a THREE.Group instance, not a Mesh.
// try logging `duckModel` variable right before assignent
// reycaster supposed to work only on meshes, but it works on group I guess

// raycaster by default is going to test also a children, and it will go recursivelly
// it just works with models

// we can deactivate recursive, by setting the second parameter of `intersectObject`/`intersectObjects` to be false

// try doing it, and intersectiong will not work for our model
// children will not be tested

// - ARRAY OF INTERSCTS

// testing single object with intersectObject will produce array, and array will have only one member
// if we have intersecting
// this is because of mentioned recursive thing
// there might be multiple meshes and meshes like donut or pipe where you will have multiple intersections when ray comes through
// doesn't need to be donut, it can be cammel where ray goes throug cammel head, but also hits cammels hump
